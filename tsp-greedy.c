...
void nn_tsp(graph g, size_t *tour)
{
	bool used[g.n];
	...
	for (size_t i = 1; i < g.n; ++i)
	{
		...
		for(size_t j = 0; j < g.n; ++j)
		{
			if(used[j])
				continue;
			if(best == -1 || graph_get_dist(g, tour[i-1],j) < graph_get_dist(g, tour[i-1], best))
				best = j;
		}
		tour[i] = best;
		used[best] = true;
...
int greedy_edge_cmp(const void *a, const void *b) 
{
        greedy_edge *a2 = (greedy_edge *) a;
        greedy_edge *b2 = (greedy_edge *) b;

        return graph_get_dist(current_graph, a2->from, a2->to) - graph_get_dist(current_graph, b2->from, b2->to);
}
...
void greedy_tsp(graph g, size_t *tour)
{
	nn_tsp(g, tour);
	int nexts[g.n];
	int prevs[g.n];
	...
	greedy_edge *edges = malloc(g.n*g.n * sizeof(greedy_edge));

	for(size_t i = 0; i < g.n; ++i)
		for(size_t j = 0; j < g.n; ++j)
		{
			edges[i*g.n+j].from = i;
			edges[i*g.n+j].to = j;
		}

	qsort(edges, g.n*g.n, sizeof(greedy_edge), &greedy_edge_cmp);

	size_t assigned = 0;

	for(size_t i = 0; i < g.n*g.n; ++i)
	{
		greedy_edge e = edges[i];
		if(e.from == e.to)
			continue;
		if(nexts[e.from] != -1 || prevs[e.to] != -1)
			continue;
		if(assigned == g.n-1 || set_union(&s[e.from], &s[e.to])) // last edge or different sets
		{
			++assigned;
			nexts[e.from] = e.to;
			prevs[e.to] = e.from;
